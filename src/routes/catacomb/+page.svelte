<script lang="ts">
    ...
    import Torch from "$lib/Torch.svelte";
    ...
    let story=`You are visiting forbidden catacond. you have only 8 fire torch (🔥), when you explore the catacomb you spend a 🔥, when the 🔥is over you will died`;
    ...
    function hoAncoraTorce():Boolean{
        for (let t of torce)
            if (torce.accesa) return true;
    ...
    let feed=[
        ...
        {
            nome:'Zombie',
            image: 'game-icons:shambling-zombie',
            hit:2,
            dmg:2,
            gld:1,
        ...
        {
            nome:'Ghoul',
            image: 'game-icons:half-body-crawling',
            hit:2,
            dmg:3,
            gld:2,
            type:typeInc.Creature,
        ...
        {
            nome:'Lich',
            image: 'game-icons:crowned-skull',
            hit:5,
            dmg:4,
            gld:10,
            type:typeInc.Creature,
    ...
    let mercato=[
       ...
       {
        nome:"Torch",
        image:'game-icons:torch',
        price:4,
        condition:()=>{return gamephases.Rest},
        effect:()=>{monete=monete-4; mercato[3].buyed=true},
        using:()=>{addTorcia(); mercato[3].buyed=false },
        buyed:false,
        decsription:"Add a Torch"
       },
       {
        nome:"Sword",
        image:'vaadin:sword',
    ...
    let torce:any=[
        {y:-12,x:-15,accesa:true},
        {y:12,x:-16,accesa:true},
        {y:42,x:-15,accesa:true},
        {y:72,x:-16,accesa:true},
        {y:-15,x:83,accesa:true},
        {y:18,x:82,accesa:true},
        {y:45,x:83,accesa:true},
        {y:70,x:82,accesa:true},
    ];
    let torce2:any=[
        {y:-12,x:-34,accesa:true},
        {y:12,x:-35,accesa:true},
        {y:42,x:-36,accesa:true},
        {y:72,x:-34,accesa:true},
        {y:-15,x:61,accesa:true},
        {y:18,x:62,accesa:true},
        {y:45,x:61,accesa:true},
        {y:70,x:60,accesa:true},
    ];
    let torce3:any=[
        {y:-12,x:-10,accesa:true},
        {y:12,x:-11,accesa:true},
        {y:42,x:-12,accesa:true},
        {y:72,x:-11,accesa:true},
        {y:-15,x:86,accesa:true},
        {y:18,x:87,accesa:true},
        {y:45,x:86,accesa:true},
        {y:70,x:87,accesa:true},
    ];
    ...
    function resiza(){
        if (window.innerWidth<800)  copiaTorce(torce2);
        else if (window.innerWidth<1200)  copiaTorce(torce3);
    }
    ...
    function spegniTorcia(){
        for (let i=torce.length-1;i>=0;i--){
            if (torce[i].accesa){
                torce[i].accesa=false;
                break;
            } 
        }
...
<div class="area"> 
    ...
    <nav>
        <div class="titolo">CATACOMBS</div>
    ...
    <div class="arena">
        ...
        {#each torce as trc, i }
            {#if trc.accesa}
                    <div class="torcia" in:fade={{
                        duration:1000
                    }}
                    out:fade={{
                        duration:2000
                    }} 
                    style="top:{trc.y}%;left:{trc.x}%;">
                        <Torch></Torch>
                    </div>    
            {/if}
        {/each}
        ...
        {:else if (game==gamephases.Win)}
            ...
            <div class='story' in:fade >Excape from catacombs with your money. You are alive and can live one day more</div>
        ...
        {:else if (game==gamephases.Lose)}
            ...
            <a data-sveltekit-preload-data="off" href="/" class='story'><button class="back">Return Home</button></a>
...
<style>
    ...
    .torcia{
     ...
     .area{
        margin-inline: 5%;
        background-image: url('/bg/crypt.svg');
